...
- In `chunk` mode, exhaustively inventory the assigned PDF pages or text lines and return only candidates supported there. In synthesis `complete` mode, trust the supplied chunk inventories, merge their true duplicates, preserve every distinct candidate and all accumulated source scopes, and do not reread the original files. In non-chunked `complete` mode, read the supplied source scopes directly. Return the complete graph, not a patch.
