...
Original sources and validated JSON are durable. Rendered HTML, answer previews, quiz exchanges, and chunk checkpoints are disposable. Validated graphs commit immediately and lead to a suggested first concept, without creating learning material. Each chat keeps its own project context and temporary previews, while validated workspace files remain shared, so multiple chats can work without a global topic selection. Specialists never write files or address the learner; the session manager validates their output before persistence.
...
│   └── graph-autocommit.md           Graph commit/recommendation regression
